...
inp = """Blueprint 1: Each ore robot costs 4 ore. Each clay robot costs 4 ore. Each obsidian robot costs 4 ore and 12 clay. Each geode robot costs 4 ore and 19 obsidian.
...
for i in inp:
    o = re.findall("\d+" ,i)
    blueprints.append(list(map(int, o)))
#name ore clay obOre obsClay geodeOre geodeObs
for bp in blueprints[0:1]:
    ore = (1,0)
    clay = (0,0)
    obsidian = (0,0)
    geode = (0,0)
    ...
    vierailtu = set()
    vierailtu.add(((1,0),(0,0),(0,0),(0,0)))
    jono = deque()
    jono.append(((1,0),(0,0),(0,0),(0,0)))
    ajat = {((1,0),(0,0),(0,0),(0,0)):1}
    #print(ajat)
    maxgeode = 0
    while len(jono) > 0:
        ore,clay,obsidian,geode = jono.popleft()
        aikanyt = ajat[(ore,clay,obsidian,geode)]
        if aikanyt == 26: break
        #naapurit

        ore = ore_tee(ore)
        clay = ore_tee(clay)
        obsidian = ore_tee(obsidian)
        geode = ore_tee(geode)
        #no build
        nval = (ore,clay,obsidian,geode)
        if (ore,clay,obsidian,geode) not in vierailtu:
            jono.append(nval)
            vierailtu.add(nval)
            ajat[nval] = aikanyt + 1

        #geode built
        if ore[1] >= bp[5] and obsidian[1] >=  bp[6]:
            ngeode = bot_lisaa(geode)
            nore = ore_vahenna(ore,bp[5]) 
            nobsidian = ore_vahenna(obsidian,bp[6])
            nval = (nore,clay,nobsidian,ngeode)
            if (nval) not in vierailtu:
                jono.append(nval)
                vierailtu.add(nval)
                ajat[nval] = aikanyt + 1

        #obsidian built
        if ore[1] >= bp[3] and clay[1] >=  bp[4]:
            nobsidian = bot_lisaa(obsidian)
            nore = ore_vahenna(ore, bp[3])
            nclay = ore_vahenna(clay, bp[4])
            nval = (nore,clay,nobsidian,geode)
            if (nval) not in vierailtu:
                jono.append(nval)
                vierailtu.add(nval)
                ajat[nval] = aikanyt + 1
        ...
        #clay built
        if ore[1] >= bp[2]:
            nclay= bot_lisaa(clay)
            nore = ore_vahenna(ore, bp[2])
            nval = (nore,nclay,obsidian,geode)
            if (nval) not in vierailtu:
                jono.append(nval)
                vierailtu.add(nval)
                ajat[nval] = aikanyt + 1
        ...
        if ore[1] >= bp[1]:
            nore = bot_lisaa(ore)
            nore = ore_vahenna(ore, bp[1])
            nval = (nore,clay,obsidian,geode)
            if (nval) not in vierailtu:
                jono.append(nval)
                vierailtu.add(nval)
                ajat[nval] = aikanyt + 1
        ...
        if aikanyt == 25:
            if geode[1] > maxgeode:
                maxgeode = geode[1]
                maxall = (ore,clay,obsidian,geode)
        print("minute",aikanyt)
        print(f"{ore=}")
        print(f"{clay=}")
        print(f"{obsidian=}")
        print(f"{geode=}")
